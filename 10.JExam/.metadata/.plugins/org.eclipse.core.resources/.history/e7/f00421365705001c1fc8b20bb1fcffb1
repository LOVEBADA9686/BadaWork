...
public class KbdBoard extends JFrame implements ActionListener {
	...
	public void kboardInsert(String bsubject, String bwriter, String bpw, String bcontents) {
		System.out.println("KbdBoard.kboardInsert() 함수 시작 >>> : ");
		
		KbdBoardService ks = new KbdBoardServiceImpl();
		KbdBoardVO kvo = null;
		kvo = new KbdBoardVO();
		kvo.setBnum(KbdBoardChabun.gubunNum());
	...
	//텍스트 클리어 
	public void valueCler() {
		System.out.println("KbdBoard.valueClear() 함수 시작 >>> : ");
		for(int i=0; i<jt.length; i++) {
			jt[i].setText("");
		}
		jpf.setText("");
