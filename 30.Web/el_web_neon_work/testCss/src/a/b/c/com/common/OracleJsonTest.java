...
public class OracleJsonTest {
	...
	public static boolean jsonPares(String fileName, String jsonVal){
	...
	public static void main(String args[]){
		try{
			...
			if(tableName !=null && tableName.length() > 0){
				...
				if(s.length() > 0) {
					boolean bool = OracleJsonTest.jsonPares(tableName, s);
					
					if(bool){
						File f = new File(CommonJSON.JSON_FILE_PATH+"/"+tableName.toLowerCase()+".json");
						System.out.println("/n" + f.getName() + "파일이 잘 생성되었습니다. ");
					}else{
						System.out.println("파일이 잘 생셩 되지 않았습니다.");
					}
				}else{
