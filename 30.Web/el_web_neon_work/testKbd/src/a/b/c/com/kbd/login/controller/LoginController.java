package a.b.c.com.kbd.login.controller;
...
import a.b.c.com.kbd.login.service.LoginService;
import a.b.c.com.kbd.login.service.LoginServiceImpl;
...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		response.setContentType("text/html; charest=EUC-KR");
		...
		if(login_type !=null && login_type.length() > 0) {
			...
			if("LOGIN".equals(login_type)){
				...
				if(nCnt == 1){
					...
					RequestDispatcher rd = request.getRequestDispatcher("/kbd/login/login.jsp");
					rd.forward(request, response);
				}else{
					out.println("<script>");
					out.println("alert('로그인 실패 !!!')");
					out.println("location.href='/testKbd/kbd/login/login.html'");
					out.println("</script>");
				}
			}
			...
			if("LOGOUT".equals(login_type)){
				...
				if(bool){
					RequestDispatcher rd = request.getRequestDispatcher("/kbd/login/logout.jsp");
					rd.forward(request, response);
				}else{
				}
			}
