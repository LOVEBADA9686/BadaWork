	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if("DOK".equals(isudtype)){
			...
			if(nCnt > 0){
				...
				request.setAttribute("nCnt", new Integer(nCnt));
				RequestDispatcher rd= request.getRequestDispatcher("/kbd/book/bookDelete.jsp");
				rd.forward(request, response);
		...
		if("I".equals(isudtype)){
			...
			if(nCnt > 0) {
				...
				RequestDispatcher rd= request.getRequestDispatcher("/kbd/book/bookInsert.jsp");
				rd.forward(request, response);
				
			}else{
				System.out.println("도서 정보 등록 실패 !!!");
				out.println("<script>");
				out.println("location.href='/testKbd/book/book.html'");
				out.println("</script>");
			}
		}	
	}else{
