package a.b.c.com.kbd.mem.controller;
...
// @ : 어노테이션 키워드
// WebServlet : 웹서블릿 어노테이션 인터페이스 
// "/mem" : 다른 페이지에서(클라이언트 사이드 스크립트, 서버사이드 스크립트)에서 이 서블릿을 찾아오는 가상경로 이름 
// " " : 문자열
// / : Context 명 
// mem : 가상 경로명 : 이것을 맘대로 변경해도 된다. 이 컨텍스트 안에서는 유니크 해야 한다. 
@WebServlet("/mem")
		...
		if (isudType !=null && isudType.length() > 0) {
			...
			if("I".equals(isudType)){
				...
				if(bFile){
					...
					System.out.println("MemberController 회원 등록 >>> : I");
					...
					if(bInsert){
						...
						RequestDispatcher rd = request.getRequestDispatcher("/kbd/mem/memInsert.jsp");
						rd.forward(request, response);
					}else{
						out.println("<script>");
						out.println("location.href='/testKbd/kbd/mem/mem.html'");
						out.println("</script>");
					}
					
				}else{
					System.out.println("회원등록 및 파일 업로드 실패");
				}
			}
		} // 회원 등록 end
			...
			if("SALL".equals(isudType)){
				...
				if(aListAll !=null && aListAll.size() > 0) {
					...
					RequestDispatcher rd = request.getRequestDispatcher("/kbd/mem/memSelectAll.jsp");
					rd.forward(request, response);
				}else {
					out.println("<script>");
					out.println("location.href='/testKbd/mem?ISUD_TYPE=SALL'");
					out.println("</script>");
				}
			}// 전체 회원정보 조회 end
			...
			if("S".equals(isudType) || "U".equals(isudType) || "D".equals(isudType)){
				...
				if(mnum !=null && mnum.length() > 0){
					...
					if(aListS !=null && aListS.size() > 0) {
						...
						RequestDispatcher rd = request.getRequestDispatcher("/Kbd/mem/memSelect.jsp");
						rd.forward(request, response);
