		...
		if(isudtype !=null && isudtype.length() > 0){
			...
			if("SALL".equals(isudtype)){
				...
				if(aList !=null && aList.size() > 0){
					...
					// /testKbd/WebContent/cgiTest/dispather.jsp
					// /testKbd/cgiTest/dispather.jsp 
					// ::WebContent 경로를 이클립스 다이나믹 웹 프로젝트 빌드 프레임워크에서 사용하는 가상 경로이다.
					// RequestDispatcher 인터페이스에서 사용하는 getRequestDispatcher 함수에는 
					// Context 경로를 사용하지 않음
					...
					out.println("location.href='/testKbd/location'");
					out.println("</script>");
				}
			}
			...
			if("S".equals(isudtype)){
				...
				if(1 == nCnt){
					...
					// RequestDispathcher 인터페이스 에서 사용하는 getRequestDispatcher 함수에서는
					// Context 경로를 사용하지 않음
					RequestDispatcher rd = req.getRequestDispatcher("/cgiTest/dispatherSelect.jsp");
				}else{
					out.println("<script>");
					out.println("alert('데이터 조회 실패')");
					out.println("location.href='/testKbd/cgiTest/dispather.html'");
					out.println("</script>");
				}
			}
